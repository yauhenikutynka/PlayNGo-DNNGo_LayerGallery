...
namespace DNNGo.Modules.LayerGallery
{
    public class Resource_jQueryFileUploadHandler : IHttpHandler
    {
        ...
        {
            get { return UserInfo != null && UserInfo.UserID > 0 && (IsAdministrator || DotNetNuke.Security.Permissions.ModulePermissionController.HasModuleAccess(DotNetNuke.Security.SecurityAccessLevel.Edit, "EDIT", ModuleConfiguration)); }
		...
		private void ListCurrentFiles (HttpContext context) {
            ...
            //int RecordCount = 0;
            //qp.Where.Add(new SearchParam(DNNGo_LayerGallery_Files._.AlbumID, WebHelper.GetStringParam(context.Request, "AlbumID", "0"), SearchType.Equal));

            //var files = DNNGo_LayerGallery_Files.FindAll(qp, out RecordCount).Select(f => new Resource_FilesStatus(f, PortalSettings)).ToArray();
